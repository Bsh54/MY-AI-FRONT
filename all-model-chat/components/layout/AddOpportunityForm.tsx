...
export const AddOpportunityForm: React.FC<AddOpportunityFormProps> = ({ onClose, onAdd }) => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Supprimer cette opportunité définitivement ?')) {
      const updated = customOpps.filter(o => o.id !== id);
      setCustomOpps(updated);
      localStorage.setItem('custom_opportunities', JSON.stringify(updated));
      onAdd([...updated, ...OPPORTUNITIES_DATA]);
    }
